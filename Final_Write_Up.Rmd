...
Because our main goal is to determine the most important factors which impact premium, we decided to employ tree models. We ran both simple decision tree and random forest models.
...
partialPlot(premium_forest, premium_test, 'shoe_size', las=1, main=paste("Partial Dependence on Shoe Size"))
...
The next most important variable was \emph{Sneaker Name}, this refers to the specific style of shoe. This captures buyers' preferences for one sneaker style over the other. We also noticed that release date was  captured using this variable since each style has a different release date. 

The color and material variables \emph{Primary Color},\emph{Material}, and \emph{Secondary Color} were the next most important, this isolates any effects color and material may have on buyer preferences that isn't captured by the name of the style. 

\emph{Brand} was the next most important variable, this attempts to isolate any brand preferences a consumer may have. The importance of this variable may be under-estimated because a lot of the effect brand may have on premium has been captured in the \emph{Sneaker Name} variable.
